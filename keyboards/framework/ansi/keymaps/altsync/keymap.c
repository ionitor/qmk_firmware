...
#include "../../../numpad/keymaps/altsync/led_enum.h"
#include "../../../numpad/keymaps/altsync/features/led_comm.h"
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        // Send messages to numpad to sync alt state
        case KC_LALT:
        case KC_RALT:
            if (record->event.pressed) {
                send_led_cmd(PRESS_ALT);
            } else {
                send_led_cmd(RELEASE_ALT);
            }
            return true; // Continue processing key as normal
            break;
